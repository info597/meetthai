...
class PushService {
  ...
  static Future<void> syncTokenForCurrentUser() async {
    ...
    try {
      ...
      if (_lastSyncedUserId == user.id && token == _cachedToken) {
        debugPrint('Push Token bereits synchronisiert');
        return;
      }
  ...
  static void _openChatFromPayload({
    ...
    if (isDuplicateOpen || _navigationInProgress) {
    ...
    Future<void> doNavigate({int retry = 0}) async {
      final navigator = appNavigatorKey.currentState;
      final context = appNavigatorKey.currentContext;

      if (navigator == null || context == null) {
        if (retry >= 10) {
          debugPrint('Navigator für Push-Navigation nicht verfügbar');
          _navigationInProgress = false;
          return;
        }
